...
class SourceForm(tk.Frame):
    def __init__(self, master, source_number, *args, **kwargs):
        ...
        self.name_entry.insert(0, random.choice(source_names_examples))
        self.name_entry.grid(row=1, column=1)

        # Signal type combo
        ttk.Label(self, text="Type:").grid(row=1, column=2, sticky='e')
        self.type_combo = ttk.Combobox(self, values=["random", "sine", "gaussian", "value_with_random_noise", "random_signal_with_noise"])
...
class SignalApp(tk.Tk):
    def __init__(self):
        ...
        self.add_button = tk.Button(self.button_frame, text="Add Source", command=self.add_source)
